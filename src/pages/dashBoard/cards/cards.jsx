    ...
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-6 px-4 w-full">
        {cards.map((item, idx) => (
          <div
            key={idx}
            className="group cursor-pointer relative p-6 rounded-xl shadow-lg overflow-hidden flex items-center justify-between bg-white transition duration-300 ease-in-out
              hover:bg-[#26a69a] hover:shadow-2xl hover:scale-[1.03]"
          ...
          >
            ...
            <div className="z-10">
              <div className="text-[#084e54] text-lg font-semibold mb-1 group-hover:text-white">{item.label}</div>
              <div className="text-3xl font-bold mb-1 text-[#084e54] group-hover:text-white">{item.value}</div>
              <div className="text-sm text-[#1d4d4f] group-hover:text-white">Increased from last month</div>
            </div>
